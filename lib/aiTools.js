 ...
 * @param {string} userId - Slack user ID (for permission filtering)
 * @returns {Array} Array of tool definitions
 */
export function getAITools(userId = null) {
  return [
    // Google Search Tool
    {
      google_search: {}
 ...
 */
function getRedmineTools(userId = null) {
  // Admin-only functions
  const adminOnlyFunctions = [
    'createRedmineIssue',
    'updateRedmineIssueStatus',
    'updateRedmineProgress',
    'addRedmineNote',
    'logRedmineTime'
  ];
  ...
  // Filter tools based on admin permission
  const filteredTools = allTools.filter(tool => {
    const toolName = tool.function?.name;
    if (adminOnlyFunctions.includes(toolName)) {
      return isAdmin;
    }
    return true;
  });

  console.log(`Tools available for user ${userId}: ${filteredTools.map(t => t.function?.name).join(', ')} (isAdmin: ${isAdmin})`);
  return filteredTools;
}

/**
 * Check if user is admin based on whitelist
...
function isUserAdmin(userId) {
  ...
  const whitelistEnv = process.env.WHITELIST_ADMIN_IDS || '';
  if (!whitelistEnv.trim()) {
    console.warn('WHITELIST_ADMIN_IDS not configured');
    return false;
  }

  const adminIds = whitelistEnv
    .split(',')
    .map(id => id.trim())
    .filter(id => id.length > 0);

  const isAdmin = adminIds.includes(userId);
  console.log(`Admin check for ${userId}: ${isAdmin} (whitelist: ${adminIds.join(', ')})`);
  return isAdmin;
}
 ...
 * @param {Array} threadMessages - Thread messages with user info (for admin mapping)
 * @returns {string} System prompt
 */
export function getSystemPrompt(vnTime, threadMessages = []) {
  let adminWarning = '';
  ...
  if (whitelistEnv.trim()) {
    ...
    if (adminIds.length > 0) {
      const adminMentions = adminIds.map(id => `anh <@${id}>`).join(', ');

      adminWarning = `\n\nADMIN IDs: ${adminIds.join(', ')}

NẾU USER YÊU CẦU CÁC HÀNH ĐỘNG SAU, HÃY TỪ CHỐI VÀ GIẢI THÍCH:
- Tạo issue mới
- Thay đổi status issue
- Update % hoàn thành issue
- Thêm comment/note vào issue
- Log thời gian làm việc

Hãy giải thích rằng chỉ ${adminMentions} mới có quyền thực hiện các hành động này. Trả lời theo ngôn ngữ tự nhiên.`;
    }
  }

  return `Bạn là một trợ lý AI thông minh và hữu ích.

THỜI GIAN HIỆN TẠI: ${vnTime}${adminWarning}
