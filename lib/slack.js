...
export async function getThreadMessages(client, channel, threadTs) {
  try {
    ...
    if (uniqueUserIds.length > 0) {
      const userInfoPromises = uniqueUserIds.map(async (userId) => {
        try {
          ...
          return { id: userId, name: userId, email: 'no-email' }; // Fallback to ID
        }
      });

      const userInfos = await Promise.all(userInfoPromises);
      userInfos.forEach(info => {
        userInfoMap[info.id] = { name: info.name, email: info.email };
      });
    }
    ...
    return result.messages.map(msg => {
      ...
      const userInfo = userInfoMap[userId] || { name: userId, email: 'no-email' };
